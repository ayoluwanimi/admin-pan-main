...
This document outlines the steps to deploy the `admin-pan-main` application, comprising a React frontend and a FastAPI backend, as a permanent website. This guide assumes deployment on a Linux server (e.g., Ubuntu) with Nginx as a reverse proxy and Gunicorn for the FastAPI application.
...
## 4. Backend Deployment (FastAPI with Gunicorn & Systemd)

### 4.1. Install Dependencies

Navigate to the `backend/` directory and install the production dependencies. It's highly recommended to use a Python virtual environment.
...
### 4.2. Configure Gunicorn

Gunicorn will serve your FastAPI application. The `admin-pan-backend.service` file (provided in the repository root) is a `systemd` unit file to manage Gunicorn.
...
### 4.3. Start and Enable the Backend Service
...
## 5. Frontend Deployment (React with Nginx)

### 5.1. Install Dependencies and Build

Navigate to the `frontend/` directory, install dependencies, and create a production build.
...
Nginx will serve the static frontend files and act as a reverse proxy for the backend API and WebSockets.
...
2.  **Build Frontend**: Run `npm install && REACT_APP_BACKEND_URL=... npm run build`.
3.  **Deploy Frontend**: Copy `build/` contents to `/var/www/admin-pan-main/frontend/build` on your server.
4.  **Build Backend**: Run `pip install -r requirements_prod.txt`.
5.  **Deploy Backend**: Copy updated backend files to `/var/www/admin-pan-main/backend`.
6.  **Restart Backend Service**: `sudo systemctl restart admin-pan-backend`.
...
-   Check Nginx and Gunicorn logs for any errors.
